...
def test_notification(host_settings, subordinate_settings, connected_forwarder, wait_for_disconnected):
    ...
    host_client.subscribe(
        [(f"foris-controller/{connected_forwarder.subordinate.controller_id}/notification/+/action/+", 0)]
    )
    assert subscribe_event.wait(TIMEOUT)

    subordinate_client.publish(
        f"foris-controller/{connected_forwarder.subordinate.controller_id}/notification/mod/action/act",
        b'{"some": "notification"}',
    )
...
def test_request(host_settings, subordinate_settings, connected_forwarder, wait_for_disconnected):
    ...
    subordinate_client.subscribe(
        [(f"foris-controller/{connected_forwarder.subordinate.controller_id}/request/+/action/+", 0)]
    )
    assert subscribe_event.wait(TIMEOUT)

    host_client.publish(
        f"foris-controller/{connected_forwarder.subordinate.controller_id}/request/mod/action/act",
        b'{"some": "request"}',
    )
...
def test_reply(host_settings, subordinate_settings, connected_forwarder, wait_for_disconnected):
    ...
    host_client.subscribe([(f"foris-controller/{connected_forwarder.subordinate.controller_id}/reply/+", 0)])
    assert subscribe_event.wait(TIMEOUT)

    reply_uuid = uuid.uuid4()

    subordinate_client.publish(
        f"foris-controller/{connected_forwarder.subordinate.controller_id}/reply/{reply_uuid}",
        b'{"some": "reply"}',
    )
